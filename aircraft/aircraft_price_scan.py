...
# Version: v0.11.0 — 2025-12-31
# Change: Barnstormers upgraded to keyword-search endpoint (better relevance,
#         less category noise). Retains flyable-only filter + bucket logic.
...
# --- Barnstormers (NEW: keyword search endpoint) ---
def fetch_barnstormers(query:str)->List[AircraftListing]:
    q = quote_plus(query)
    url = f"https://www.barnstormers.com/cat_search.php?keyword={q}"
    html = fetch_html(url,"barnstormers_latest.html")
    if not html: return []
    soup = BeautifulSoup(html,"html.parser")
    out=[]
    for card in soup.select("div.adBox, div.classified, table, td"):
        text=" ".join(card.get_text(" ",strip=True).split())
        if not text:
            continue
        if "featured ads" in text.lower():
            continue
        link = card.find("a",href=True)
        href = ("https://www.barnstormers.com"+link["href"]) if link and link["href"].startswith("/") else url
        l = make_listing("Barnstormers",query,text,href)
        if l: out.append(l)
    return out
...
def main():
    if len(sys.argv)<2:
        print('Usage: aircraft_price_scan.py "Piper Cherokee 140" [--save] [--IFR]')
        sys.exit(1)

    query=sys.argv[1]
    save="--save" in sys.argv
    ifr_only="--IFR" in sys.argv
    ...
    collect("Barnstormers", lambda: fetch_barnstormers(query))
    ...
    if ifr_only:
        listings=[l for l in listings if l.bucket!="vfr_basic"]
