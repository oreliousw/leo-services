...
def oanda_get_pip_value(instrument: str) -> float:
    r = oanda_session.get(
        f"{OANDA_REST_URL}/v3/instruments/{instrument}",
        timeout=10
    )
    return 10 ** float(r.json()["instrument"]["pipLocation"]) if r.ok else 0.0001

def oanda_place_market_order(instrument: str, units: int, sl: float, tp: float, tag: str):
    side = "BUY" if units > 0 else "SELL"
    logging.info(f"[MES] Placing {side} {instrument} {abs(units)} units | SL={sl:.5f} TP={tp:.5f}")
...
def estimate_units_for_risk(instrument: str, direction: str, nav: float, risk_pct: float, entry: float, sl: float) -> int:
    ...
    pip_size = oanda_get_pip_value(instrument)
...
def evaluate_pair(instrument: str, base_risk_pct: float, nav: float, open_pos: Dict[str, float]) -> MesDecision:
    ...
    atr_pips = atr_val / oanda_get_pip_value(instrument)
    sl_pips = sl_multiplier * atr_pips
    tp_pips = round(np.random.uniform(*tp_pips_range), 1)

    entry = df1h["close"].iloc[-1]
    sl_price = entry - sl_pips * oanda_get_pip_value(instrument) if direction == "bullish" else entry + sl_pips * oanda_get_pip_value(instrument)
