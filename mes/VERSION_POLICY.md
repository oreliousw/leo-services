...
- `strategy_behavior` + `hotfix_critical` → apply **SUFFIX** instead of **MINOR**  
  (allows emergency behavior fixes without forcing a MINOR bump)
...
- Version governance applies **only** to Git tags/branches — never affects live execution
- Changes to this policy file itself **do not** trigger MES version increments
