...
# ============================================================
# File: kraken_eth.py
# Version: v2.6 — Per-Asset USD Slice Autopilot (ETH/USD)
#
# Mode: LIVE — MARKET BUY/SELL (signals-free execution)
# Asset-in-hand mode supported:
#   • If ETH balance > 0 and mode=idle → treat as HOLD position
...
def k_private(path, params):
    ...
    req.add_header("User-Agent", "Kraken-MES-eth-v2.6")
...
def load_state():
    ...
    if base["usd_slice"] is None:
        try: base["usd_slice"] = float(ETH_USD_SLICE_INIT or 0)
...
def maybe_send_heartbeat(state, price):
    ...
    tg_send(
        ...
        f"Price: {price:.2f}\n"
        f"Swing High: {state.get('last_swing_high',price):.2f}"
    )
...
def execute_buy(price,state):
    ...
    log_event({"event_type":"eth_buy","price":price,"volume":volume,"usd_spent":usd_to_spend})
    return True
...
def execute_sell(reason,price,state):
    ...
    log_event({"event_type":f"eth_sell_{reason}","price":price,"volume":volume,"notional":notional})
    return True
...
def engine_tick():
    ...
    detect_initial_position(s, eth_bal, price)
...
if __name__=="__main__":
    try:
        print("Kraken ETH Trader v2.6 tick OK"); engine_tick()
    except Exception as e:
        tg_send(f"❌ Kraken ETH v2.6 runtime error:\n{e}")
